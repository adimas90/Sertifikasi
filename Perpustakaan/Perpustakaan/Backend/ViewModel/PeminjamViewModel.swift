...
class PeminjamViewModel: ObservableObject {
  ...
  private func addPeminjam(_ peminjam: IdentitasPeminjam) {
    do {
      let _ = try db.collection("transaksi_pinjam").addDocument(from: peminjam)
    }
    catch {
      print(error)
    }
  ...
  private func updatePeminjam(_ peminjam: IdentitasPeminjam) {
    if let documentId = peminjam.id {
      do {
        try db.collection("transaksi_pinjam").document(documentId).setData(from: peminjam)
      }
      catch {
        print(error)
      }
    ...
    if let _ = identitas_peminjam.id {
      self.updatePeminjam(self.identitas_peminjam)
        ...
        do{
            ...
            let idBook = try db.collection("transaksi_pinjam").addDocument(from: identitas_peminjam)
            idBook.setData([
